...
class TestConfigVRPDriver(unittest.TestCase, TestConfigNetworkDriver):
    """Configuration Tests for VRPDriver.
    ...
    def test_huawei_vrp_only_gen_full_path(self):
...
class TestGetterVRPDriver(unittest.TestCase, TestGettersNetworkDriver):
    """Getters Tests for VRPDriver.
    ...
    def setUpClass(cls):
        ...
        if cls.mock:
            cls.device.device = FakeVRPDevice()
        else:
            cls.device.open()

    def test_huawei_vrp_only_bgp_time_conversion(self):
...
class FakeVRPDevice:
    ...
    def send_command_expect(self, command, **kwargs):
        ...
        output = self.read_txt_file("huawei_vrp/mock_data/{}.txt".format(cmd))
        return str(output)
